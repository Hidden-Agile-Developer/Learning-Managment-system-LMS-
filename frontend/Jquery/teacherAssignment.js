...
$.ajax({
  ...
  $("#assignment_upload").click(function(){         
   ...
   $.ajax({
   url:'http://localhost:96/addTechAssignment/',
   type:'post',
   dataType:'json',
   data:data,
...
$.ajax({  
    type: 'get',
    dataType : 'JSON',
    url: 'http://localhost:96/viewAssignmentTeacher',
    success: function(data) {
$.each(data,function(index){
    ...
    "<td class='col-xs-1'>"+ "<a href='updateAssignment.html?id="+data[index]._id+"' class='btn btn-primary btn-sm'>"+
    "Edit</a></td>"+
...
function update_assign(){
   ...
   $.getJSON("http://localhost:96/viewAssignmentTeacher/" + id, function (result) {
    ...
    $.ajax({
       url:'http://localhost:96/updateAssignment/' + id,
       type:'PUT',
       dataType:'json',
       data:data,
       success:function(res, textStatus, xhr){
...
function delete_assign(){
    ...
    id=$(this).attr('assign_id');
       $.ajax({
        type: 'DELETE',
        url: 'http://localhost:96/deleteAssignment/'+id,
